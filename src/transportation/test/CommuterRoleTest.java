...
public class CommuterRoleTest extends TestCase {
	...
	public void testBusCommuterScenario()
	{
		...
		assertEquals("commuterRole should be in the walking state, but he's not.", PersonState.walking, commuterRole.mState);
		
		//used to notify the bus
		commuterRole.mDestinationBusStop = 1;
		commuterRole.mState = PersonState.atBusStop;
		...
		assertEquals("The commuterRole state should set to boardingBus, but it isn't.", PersonState.boardingBus, commuterRole.mState);
		...
		commuterRole.mState = PersonState.ridingBus;
		...
		assertEquals("The commuterRole state should set to ridingBus, but it isn't.", PersonState.ridingBus, commuterRole.mState);
		
		//mockBus has reached destination
		commuterRole.msgAtStop(1);
		
		assertEquals("The commuterRole state should set to exitingBus, but it isn't.", PersonState.exitingBus, commuterRole.mState);
		assertTrue("The commuterRole should have run, it did not.", commuterRole.pickAndExecuteAnAction());
		...
		commuterRole.mState = PersonState.walking;
		
		assertEquals("The mockBus should have three events in it's event log, but it doesn't.",3, mockBus.log.size());
		assertEquals("The commuterRole state should set to walking, but it isn't.", PersonState.walking, commuterRole.mState);
