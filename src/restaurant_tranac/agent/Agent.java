package restaurant_tranac.agent;
...
public abstract class Agent {
    ...
    /* Message called from gui. Looks at the paused boolean and 
     * decides to pause or restart agent.
     */
    public void pauseRestartAgent() {
    	if(paused) {
    		paused = false;
    		pauseAction.release();
    	}
    ...
    private class AgentThread extends Thread {
        ...
        public void run() {
            ...
            while (goOn) {
            	// If the agent is to be paused, it tries to acquire a semaphore and goes
            	// to sleep until it's released (by the gui).
            	while(paused) {
            		try {
            			pauseAction.acquire();
            		}
